...
| T15 | No-placeholder / custom-axiom / `#print axioms` audit | A/CI: fail-closed scans reject `sorry`, `admit`, and custom `axiom` declarations; `Main.lean` prints axioms for the proof-critical theorem inventory. Final status remains FAIL until the final HEAD reaches and passes the axiom-report step. | FAIL |
